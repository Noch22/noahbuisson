...
const initHeaderMenu = () => {
  ...
  const getExpandedWidth = () => elements.length * 100 + (elements.length - 1) * 16;
  ...
  const buildIntroTimeline = (tl) => {
    ...
    elementSpans.forEach((el, i) => {
      const split = new gsap_SplitText__WEBPACK_IMPORTED_MODULE_0__.SplitText(el, { type: "chars" });
      tl.from(split.chars, {
        delay: i * 0.02,
        opacity: 0,
        y: 20,
  ...
  const playIntroOnFirstLoad = () => {
    if (!shouldPlayIntro() || isIntroRunning)
      return;
    stopActiveAnimation();
    isIntroRunning = true;
    activeTimeline = gsap.timeline({
...
(_a = window.__headerMenuCleanup) == null ? void 0 : _a.call(window);
initHeaderMenu();
...
/******/ 	(() => {
/******/ 		__webpack_require__.h = () => ("d23465feb4c7cbc789c9")
/******/ 	})();
/******/ 	
